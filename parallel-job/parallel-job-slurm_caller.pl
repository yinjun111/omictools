...
Usage: omictools parallel-job -i yourscripts.sh -n yourscripts -o jobsubmissionfolder -t 5 --nodes 1 --ppn 4 -r
...
omictools parallel-job -k 600 -r


Description: In HPC cluster, use multiple controled tasks for paralleling.
...
#In Falco, BSR Linux cluster, use screen+parallel to parallelly running jobs in background.
...
if($runmode) {
	print STDERR "Submitting cluster jobs, running $runcommand.\n\n" if $verbose;
	print LOG "Submitting cluster jobs, running $runcommand.\n\n";
	system($runcommand);
	print STDERR "Done.\n\n" if $verbose;
	print LOG "Done.\n\n";
}
else {
	print STDERR "In cluster, run $runcommand.\n\n" if $verbose;
	print LOG "In cluster, run $runcommand.\n\n";
}
...
print OUT2 "#SBATCH --job-name=",$commandline_script,"\n";
...
if(defined $params->{"ppn"}) {
	if(defined $params->{"nodes"}) {
		#print OUT2 "## Parallel environment to define # of cores\n#PBS -l nodes=",$params->{"nodes"},":ppn=",$params->{"ppn"},"\n";
		print OUT2 "## Parallel environment to define # of cores\n#SBATCH -N ",$params->{"nodes"}," -n ",$params->{"ppn"},"\n";
	}
	else {
		#print OUT2 "## Parallel environment to define # of cores\n#PBS -l nodes=1:ppn=",$params->{"ppn"},"\n";
		print OUT2 "## Parallel environment to define # of cores\n#SBATCH -N 1 -n ",$params->{"ppn"},"\n";
	}
}
else {
	if(defined $params->{"nodes"}) {
...
#	print OUT2 "## Parallel environment to define # of procs\n#PBS -l procs=",$params->{"procs"},"\n";
#}

if(defined $params->{"ncpus"}) {
	#print OUT2 "## Parallel environment to define # of cpus\n#PBS -l ncpus=",$params->{"ncpus"},"\n";
	print OUT2 "## Parallel environment to define # of cpus\n#SBATCH -c ",$params->{"ncpus"},"\n";
}

if(defined $params->{"mem"}) {
	#parallelcluster doesn't support mem usage for slurm

	#print OUT2 "#PBS -l mem=",$params->{"mem"},"\n"; 
	#print OUT2 "## Set memory per node\n#SBATCH --mem ",$params->{"mem"},"\n";
}
...
print OUT2 "#SBATCH -o ",$params->{"oo"},"/$commandline_script.out.txt\n";
print OUT2 "#SBATCH -e ",$params->{"eo"},"/$commandline_script.err.txt\n";
